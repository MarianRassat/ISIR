...
namespace RT_ISICG
{
	...
	class Scene
	{
	  public:
		...
		void loadFileTriangleMesh( const std::string & p_name, const std::string & p_path, const Vec3f p_position, const float p_scale );
		...
		void addObject( BaseObject * p_object ) { _addObject( p_object ); }
		void addMaterial( BaseMaterial * p_material ) { _addMaterial( p_material ); }
		void attachMaterialToObject( const std::string & p_materialName, const std::string & p_objectName ) { _attachMaterialToObject( p_materialName, p_objectName ); }
