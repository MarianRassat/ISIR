...
namespace RT_ISICG
{
	bool Cube::intersect( const Ray & p_ray, const float p_tMin, const float p_tMax, HitRecord & p_hitRecord ) const
	{
		float t1;
		float t2;
	...
	bool Cube::intersectAny( const Ray & p_ray, const float p_tMin, const float p_tMax ) const
	{
		float t1;
		float t2;
		if ( _geometry.intersect( p_ray, t1, t2 ) )
